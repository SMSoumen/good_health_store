...
@endphp
<div id="page-content" class="mb-5" wire:ignore>
    ...
    <div class="container-fluid">
            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 col-12 product-layout-info">
                    <div class="product-single-meta">
                        ...
                        @php
                            // Simple/combo products always show their SKU. Variable products
                            // fall back to the product SKU (if any) and are updated to the
                            // selected variant's SKU via JS when a variant is chosen.
                            $initial_sku = !empty($product_details->sku)
                                ? $product_details->sku
                                : ($product_details->type != 'variable_product'
                                    ? $product_details->variants[0]['sku'] ?? ''
                                    : '');
                        @endphp
                        <p class="product-sku mb-10px {{ empty($initial_sku) ? 'd-none' : '' }}"
                            id="product-sku-wrapper">{{ labels('front_messages.sku', 'SKU') }}:<span
                                class="text fw-500" id="product-sku-value">{{ $initial_sku }}</span></p>
                        <div class="d-flex align-items-start justify-content-between flex-wrap gap-3 mb-10px">
                        <div class="product-swatches-option mb-0 flex-grow-1">
                        ...
                        @php
                            $isSimpleProduct = $product_details->product_type == 'simple_product';
                            if ($isSimpleProduct) {
                                // no stock tracking -> treat as available
                                $stockAvailability =
                                    $product_details->stock_type === '' || $product_details->stock_type === null
                                        ? 1
                                        : ($product_details->availability === ''
                                            ? 1
                                            : (int) $product_details->availability);
                            } else {
                                // variable product -> in stock if ANY variant is available
                                $stockAvailability = collect($product_details->variants)->contains(function ($v) {
                                    return ($v['stock_type'] ?? '') === '' ||
                                        (int) (($v['availability'] ?? '') === '' ? 1 : $v['availability']) >= 1;
                                })
                                    ? 1
                                    : 0;
                            }
                        @endphp
                        <p class="product-stock d-flex align-items-center mb-0" id="product-stock-status">
                            <span class="pro-stockLbl ps-0">
                                <span
                                    class="d-flex-center align-items-center stockLbl instock text-uppercase {{ $stockAvailability >= 1 ? '' : 'd-none' }}">
                                    <ion-icon name="checkmark-circle"
                                        class="text-success fs-5 me-1"></ion-icon>{{ labels('front_messages.in_stock', 'In stock') }}
                                </span>
                                <span
                                    class="d-flex-center align-items-center stockLbl outstock text-uppercase text-danger {{ $stockAvailability >= 1 ? 'd-none' : '' }}">
                                    <ion-icon name="close-circle"
                                        class="text-danger fs-5 me-1"></ion-icon>{{ labels('front_messages.out_of_stock', 'Out of Stock') }}
                                </span>
                            </span>
                        </p>
                        </div>
                        <style>
                            .product-single .product-form-submit .btn.ghs-cart-btn {
                                height: 40px;
                                padding: 6px 10px;
                            }

                            .product-single .product-form-submit .btn.ghs-cart-btn ion-icon {
                                font-size: 22px;
                            }
                        </style>
                        <div class="product-action w-100 d-flex-wrap my-0">
                            ...
                            <div class="product-form-submit addcart fl-1 ms-3">
                                <button type="submit" name="add"
                                    class="btn btn-secondary product-form-cart-submit add_cart dlt-add-cart d-flex align-items-center justify-content-center ghs-cart-btn"
                                    ...
                                    data-brand-name='{{ app(TranslationService::class)->getDynamicTranslation(Brand::class, 'name', $product_details->brand, $language_code) }}'
                            ...
                            <div class="product-form-submit buyit fl-1 ms-3">
                                <button type="submit"
                                    class="btn btn-primary buy_now add_cart dlt-add-cart d-flex align-items-center justify-content-center ghs-cart-btn"
                                    data-product-reference-id="{{ isset($reference_id) && !empty($reference_id) ? $reference_id : '' }}"
                        ...
                        {{-- product-availability hidden per request (stock status now shown next to the variant options) --}}
                    </div>
                    <x-stickers-section :stickers="$product_details->stickers ?? []" />
                    <x-key-features-section :features="$product_details->key_features ?? []"
                        :product-name="$product_details->name" />
                    {{-- <hr class="light-hr" /> --}}
                    {{-- product-swatches-option relocated: now rendered right after the short description --}}
                    @foreach ($product_details->variants as $variant)
                        <input type="hidden" class="variants" name="variants_ids" data-image-index=""
                            ...
                            data-id="{{ $variant['id'] }}" data-sku="{{ $variant['sku'] ?? '' }}"
                            data-availability="{{ ($variant['stock_type'] ?? '') === '' ? 1 : (int) (($variant['availability'] ?? '') === '' ? 1 : $variant['availability']) }}"
                    ...
                    {{-- product-action relocated: now rendered right after the attribute selection --}}
                    ...
                    <div class="product-info">
                        ...
                        @else
                            {{-- Product policy badges (guarantee / warranty / returnable / COD / cancelable) hidden per request --}}
                        @endif
                        {{-- product-stock relocated: now shown in a single row next to the variant options --}}
                        {{-- Store name + seller rating section hidden per request
                        <hr class="light-hr" />
                        ...
                        @endphp
                        <div class="d-flex flex-wrap align-items-center gap-3 mb-10px">
                            @if (!empty($product_details->made_in))
                                <p class="product-sku pb-0 mb-0">{{ labels('front_messages.made_in', 'Made In') }}:<span
                                        class="text fw-500">{{ $product_details->made_in }}</span></p>
                            @endif
                            <a href="{{ customUrl('categories/' . $category[0]->slug . '/products') }}"
                                class="product-type fs-6 mb-0" title="{!! app(TranslationService::class)->getDynamicTranslation(
                                    Category::class,
                                    'name',
                            ...
                            @if (count($product_details->tags) >= 1)
                                <p class="text-uppercase text-black mb-0"><ion-icon name="pricetags-outline"
                        ...
                        {{-- "Check Product Deliverability" section hidden per request --}}
                        @if (false && $product_details->product_type != 'digital_product')
                            <hr class="light-hr" />
        ...
        <div class="tabs-listing section pb-0">
            <ul class="product-tabs list-unstyled d-flex-wrap border-bottom d-none d-md-flex">
                ...
                {{-- Additional Information tab hidden per request --}}
                @if (false && $product_details->attributes != [])
    ...
    @if (count($relative_products) >= 1)
        <hr class="ghs-related-separator">
        <section class="section product-slider pb-0">
            <div class="container-fluid">
                ...
                <!--Product Grid-->
                <div
                    class="swiper related-products-swiper gp15 arwOut5 hov-arrow grid-products {{ ($store_settings['products_display_style_for_web'] ?? '') == 'products_display_style_for_web_3' ? 'pro-hover3' : '' }}">
                    <div class="swiper-wrapper">
                        @foreach ($relative_products as $details)
                            <div class="swiper-slide">
                                <x-ghs.product-card :product="$details" />
                            </div>
                        @endforeach
                    </div>
    ...
    <div class="stickyCart">
        ...
        <script>
            ...
            // Related Products slider — fewer, larger cards (4 per view on desktop)
            function initRelatedProductsSwiper() {
                if (typeof Swiper === 'undefined') return;
                document.querySelectorAll('.related-products-swiper').forEach(function (el) {
            ...
            document.addEventListener('livewire:navigated', initRelatedProductsSwiper);
