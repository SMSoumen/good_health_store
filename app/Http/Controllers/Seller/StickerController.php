...
class StickerController extends Controller
{
    ...
    {
        return view('seller.pages.forms.stickers');
    }
    ...
    public function list(Request $request)
    {
        ...
        $data = $stickers->map(function ($s) {
            $editUrl = route('seller.stickers.edit', $s->id);
            $deleteUrl = route('seller.stickers.destroy', $s->id);
            ...
            return [
                ...
                'text' => $s->text,
                'operate' => $action,
                'status' => '<select class="form-select change_toggle_status ' . ($s->status == 1 ? 'active_status' : 'inactive_status') . '" data-id="' . $s->id . '" data-url="/seller/sticker/update_status/' . $s->id . '" aria-label="">' .
    ...
    public function edit($id)
    {
        ...
        if ($data === null || empty($data)) {
            return view('seller.pages.views.no_data_found');
        }

        return view('seller.pages.forms.update_sticker', ['data' => $data]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                ...
                'location' => route('seller.stickers.index')
            ]);
        }

        return redirect()->route('seller.stickers.index')->with('success', labels('admin_labels.sticker_updated_successfully', 'Sticker updated successfully'));
    }
