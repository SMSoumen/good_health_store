...
@section('content')
    <x-seller.breadcrumb :title="labels('admin_labels.stickers', 'Stickers')" :subtitle="labels('admin_labels.manage_reusable_product_stickers', 'Create reusable stickers (icon + text) to attach to products')" :breadcrumbs="[['label' => labels('admin_labels.stickers', 'Stickers')]]" />
    ...
    <div class="row">
        ...
        <div class="col-md-12 col-xl-4">
            <div class="card">
                ...
                <form action="{{ route('seller.stickers.store') }}" class="submit_form" enctype="multipart/form-data"
                    method="POST">
                    @csrf
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12 d-flex justify-content-end mt-md-0 mt-sm-2">
                                    <div class="input-group me-2 search-input-grp">
                                        ...
                                        <input type="text" data-table="seller_sticker_table"
                                            class="form-control searchInput"
                                            placeholder="{{ labels('admin_labels.search', 'Search') }}">
                                        <span class="input-group-text">{{ labels('admin_labels.search', 'Search') }}</span>
                                    </div>
                                    <a class="btn me-2" id="tableFilter" data-bs-toggle="offcanvas"
                                        data-bs-target="#columnFilterOffcanvas" data-table="seller_sticker_table"
                                        StatusFilter='true'><i class='bx bx-filter-alt'></i></a>
                                    <a class="btn me-2" id="tableRefresh" data-table="seller_sticker_table"><i
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <button type="button" class="btn btn-outline-primary btn-sm delete_selected_data"
                                data-table-id="seller_sticker_table"
                                data-delete-url="{{ route('seller.stickers.delete') }}">{{ labels('admin_labels.delete_selected', 'Delete Selected') }}</button>
                        </div>
                        <div class="col-md-12">
                            <div class="pt-0">
                                <div class="table-responsive">
                                    <table class='table' id="seller_sticker_table" data-toggle="table"
                                        data-loading-template="loadingTemplate"
                                        data-url="{{ route('seller.stickers.list') }}" data-click-to-select="true"
    ...
    <script>
        ...
        $(document).ajaxSuccess(function (event, xhr, settings) {
            var storeUrl = "{{ route('seller.stickers.store') }}";
