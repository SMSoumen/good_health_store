...
function countMisconfiguredProducts($seller_id = "", $store_id = "")
{
    ...
    // Count products with deliverable_type='specific' but no zones if Local Shipping is enabled
    if ($localShippingEnabled) {
        $count += \App\Models\Product::query()
            ->where('deliverable_type', 'specific')
            ->where(function ($q) {
                $q->whereNull('deliverable_zones')
...
function isProductMisconfigured($product)
{
    ...
    if ($localShippingEnabled) {
        ...
        if ($deliverableType === 'specific' && (empty($deliverableZones) || $deliverableZones === '[]')) {
            return true;
        }
    }
