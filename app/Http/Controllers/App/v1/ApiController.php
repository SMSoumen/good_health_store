    ...
    public function remove_from_cart(Request $request, CartController $cartController)
    {
        ...
        */
        ...
        if ($response = $this->HandlesValidation($request, $rules, [], null, true)) {
            ...
            // Get promo codes
            $promo_codes = app(PromoCodeService::class)->getActivePromoCodes($store_id);
            ...
            $response['promo_codes'] = $promo_codes;
            $response['data'] = [
    ...
    */
    ...
    public function add_transaction(Request $request, TransactionController $TransactionController)
    {
        ...
        if ($payment_method === 'stripe') {
            if (empty($transaction_type) && empty($order_id)) { $transaction_type = 'wallet'; }
            if (empty($request['type'])) { $request['type'] = 'credit'; } // Essential for wallet branch line 6136
        }
    ...
    public function ipn(Request $request)
    {
        ...
        if (!$verified) {
            ...
            return response()->json(['error' => true, 'message' => 'Invalid IPN'], 400);
        }
